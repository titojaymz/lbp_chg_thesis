...
class fbs extends CI_Controller
{
    ...
    public function index($id,$operation = NULL)
    {
        ...
        $this->load->view('fblist',$data);
        $this->load->view('footer');
    }

    public function fbadd($id)
    {
        if (!$this->session->userdata('user_data')){
            redirect('user','location');
        }
        ...
        else
        {
            ...
            $insertResult = $fbs->insert($id,$x_lastname,$x_firstname,$x_middlename,$x_extname,$x_region_id,$x_prov_id,$x_muni_city_id,$x_brgy_id);
            if($insertResult > 0)
            {
                redirect('fbs/index/'.$id.'/landaddsuccess');
            }
        }
    }
    ...
    public function fbedit($id = NULL,$redirect_id)
    {
        if (!$this->session->userdata('user_data')){
            redirect('user','location');
        }
        ...
        if (!$this->form_validation->run())
        {
            $data = array(
                ...
                'lib_regions' => $landRegList->getRegions(),
                'landclass' => $landclass->getLandClassSelect(),
                'fbdata' => $fbs->getFbData($id)
            );

            $this->load->view('header');
            $this->load->view('navbar',$data);
            $this->load->view('sidebar');
            $this->load->view('fbedit',$data);
            $this->load->view('footer');
        }
        else
        {
            ...
            $insertResult = $fbs->update($x_lastname,$x_firstname,$x_middlename,$x_extname,$x_region_id,$x_prov_id,$x_muni_city_id,$x_brgy_id,$id);
            if($insertResult > 0)
            {
                redirect('fbs/index/'.$redirect_id.'/landaddsuccess');
            }
        }
    }

    public function renderFbs($id) {

        $VolunteerProfileAdminModel = new Farmer_benes();

        $results_array = $VolunteerProfileAdminModel->getAllFbs($id);
